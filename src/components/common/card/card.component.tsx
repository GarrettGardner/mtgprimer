...
export const Card = (props: { card: ICard; stack: boolean }) => {
  ...
  const isTransformable = ["transform", "modal_dfc"].findIndex((layout) => layout === props.card.layout) > -1;
  ...
  return (
    <div className={`${styles.card}${props.stack ? " isStack" : ""}`}>
      ...
      <div>
        ...
        {isTransformable && (
          <button className={`transformButton`} title="Transform Card" onClick={() => setIsTransformed(!isTransformed)}>
