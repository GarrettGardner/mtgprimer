...
export const useFilters = (cards: ICard[], filter: IFilter) => {
  const [filterSelections, setFilterSelections] = useState<IFilterSelections>(filter.defaultSelections);
  const [groups, setGroups] = useState<ICardGroup[]>([]);

  useEffect(() => {
    const hashChange = () => {
      setFilterSelections(decodeHash(filter.defaultSelections, window.location.hash));
    };
