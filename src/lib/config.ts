...
export const generatePages = async (pageConfig: TPageConfig) => {
  ...
  const _parsePageConfig = async (pageConfig: TPageConfig, parentTemplate?: TTemplate): Promise<TTemplate[]> => {
        ...
        switch (template.type) {
          ...
          case "guide":
            if (pageConfig.template === "guide") {
              const guideData = await _parseGuideConfig(formatCode, pageConfig.guide);
          ...
          case "guide-group":
            if (pageConfig.template === "guide-group") {
              const guideGroupData = await _parseGuideGroupConfig(formatCode, pageConfig.guides, {
  ...
  const _loadGuideJson = async (formatCode: string, guideFile: string) => {
    ...
    try {
      guideConfig = (await import(`@/content/guides/${formatCode}/${guideFile}.json`))?.default;
    } catch (e: unknown) {
      console.error(e);
    }
  ...
  const _parseGuideConfig = async (
    ...
    const guideFile = guideKey === "gallery" ? formatCode : `${formatCode}-${guideKey}`;
    const preset = PRESET[`guide-${guideKey}`];

    let guideConfig = await _loadGuideJson(formatCode, guideFile);
    if (!guideConfig) return;

    let guideConfigGallery = guideKey !== "gallery" ? await _loadGuideJson(formatCode, formatCode) : undefined;

    let formatName = guideConfigGallery?.formatName ?? guideConfig.formatName ?? formatCode;
    ...
    let cards = Object.keys(guideConfig.cards).map((cardName) =>
      _mergeCards([
        { code: guideConfig.cards[cardName]?.code ?? guideConfig.code ?? guideConfigGallery?.code ?? formatCode },
        _parseCardConfig(cardName, guideConfig.cards[cardName]),
        _parseCardConfig(cardName, guideConfigGallery?.cards[cardName]),
      ]),
    );

    const includeFormats = guideConfig.includeFormats ?? guideConfigGallery?.includeFormats;
    if (includeFormats) {
    ...
    const filter = {
      ...
      defaultSelections: {
        ...
        grouping: guideConfig.defaultGrouping ?? DEFAULT_FILTER_SELECTIONS.grouping,
        ordering: guideConfig.defaultOrdering ?? DEFAULT_FILTER_SELECTIONS.ordering,
      },
    };
  ...
  const _parseGuideGroupConfig = async (
    ...
    const guidesData = (
      await Promise.all(guideKeys.map(async (guideKey) => await _parseGuideConfig(formatCode, guideKey)))
    ).filter((guide) => !!guide);
  ...
  const _mergeCards = (cards: Partial<ICard>[]): ICard => ({
    ...
    colorsRequired: cards.find((card) => !!card.colorsRequired)?.colorsRequired ?? [],
  ...
  const _parseCardConfig = (cardName: string, cardConfig?: Partial<ICardConfig>): Partial<ICard> => {
