import type { ICard, TColor, IFilter, ICardGroup, IFilterSelections } from "@/models";

export const applyFilters = (cards: ICard[], filter: IFilter, selections: IFilterSelections) =>
  _groupCards(_orderCards(_filterCards({ cards, filter, selections })));

const _filterCards = (props: { cards: ICard[]; filter: IFilter; selections: IFilterSelections }) => {
  ...
  return {
    cards: props.cards.filter((card) => {
        ...
        !card.colorsRequired.reduce(
          (acc, colorRequired) =>
            ...
            colorRequired
              .split("")
              .reduce(
                (accParts, colorRequiredPart) => accParts || selections.colors.includes(colorRequiredPart as TColor),
                false,
              ),
          true,
        )
      ...
      if (selections.rarities.length && !selections.rarities.includes(card.rarity)) {
        return false;
      }
      ...
      if (
        ...
        !card.categories.reduce((acc, category) => acc || selections.categories.includes(category), false)
      ) {
        return false;
      }
...
const _orderCards = (props: { cards: ICard[]; filter: IFilter; selections: IFilterSelections }) => {
  ...
  orderOrdering.forEach((ordering) => {
    cards.sort((a, b) => {
      switch (ordering) {
        ...
        case "rarity":
          return orders.rarity.indexOf(a.rarity) - orders.rarity.indexOf(b.rarity);
        case "number":
...
const _groupCards = (props: { cards: ICard[]; filter: IFilter; selections: IFilterSelections }) => {
  const cards = props.cards;
  const selections = props.selections;
  const options = props.filter.options;
  ...
  let groupingKey: keyof ICard = selections.grouping === "category" ? "categories" : selections.grouping;
