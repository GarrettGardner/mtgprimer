import type { TColor, TGrouping, TRarity, TOrdering, TView, IFilterSelections } from "@/models";
import { DEFAULT_FILTER_OPTIONS } from "@/constants";

export const encodeHash = (defaultFilterSelections: IFilterSelections, filterSelections: IFilterSelections) => {
  let hash = "";
  ...
  Object.keys(filterSelections).map((slug) => {
    ...
    if (filterSelections[key] && defaultFilterSelections[key]) {
      if (typeof filterSelections[key] === "string" && filterSelections[key] !== defaultFilterSelections[key]) {
        hash += `${hash ? "&" : ""}${key}:${filterSelections[key]}`;
      } else if (Array.isArray(filterSelections[key]) && filterSelections[key].length) {
...
export const decodeHash = (defaultFilterSelections: IFilterSelections, hash: string) => {
        ...
        switch (key) {
          ...
          case "colors":
            ...
            filterSelections[key] = value
              .split("+")
              .filter((item) => item in DEFAULT_FILTER_OPTIONS.colors) as TColor[];
            break;
          case "rarities":
            // TODO: Remove as
            filterSelections[key] = value
              .split("+")
              .filter((item) => item in DEFAULT_FILTER_OPTIONS.rarities) as TRarity[];
            break;
          case "sets":
          case "categories":
